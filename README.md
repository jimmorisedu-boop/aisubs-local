...
1. Запустите **`setup.bat`** — скачает портативный Python и ffmpeg (~600 МБ).
2. Запустите **`run.bat`** — откроется окно приложения.

При первой обработке скачается модель распознавания (~3 ГБ). Это разово: дальше
всё работает офлайн. Если загрузка оборвалась — просто запустите ещё раз,
докачка продолжится с места обрыва.
