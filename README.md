...
| Модель распознавания | 0.5–3 ГБ | предложит выбрать, можно пропустить |

Модель установщик предлагает на выбор: `large-v3` (~3 ГБ, максимальное
качество), `distil-large-v3` (~1.5 ГБ, быстрее и почти так же точно) или
`small` (~500 МБ). Если пропустить, модель скачается сама при первой обработке
видео — приложение при этом откроется с той моделью, которая уже есть на диске,
и не станет тянуть лишние гигабайты.
